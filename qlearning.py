...
class QLearningAgent:
    ...
    def update(self, obs, action, reward, terminated, next_obs):
        ...
        """
        next_q = (not terminated)* np.max(self.q_values[next_obs])
        updated_q = reward + self.discount_factor*(next_q - self.q_values[obs][action])
        self.q_values[obs][action]+= self.learning_rate*updated_q
